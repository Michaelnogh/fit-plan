...
**גרסה 2.0 — עיצוב "Instrument Panel"**
...
## חזרה לעיצוב הקודם (v1)

הגרסה הקודמת שמורה בענף `v1-original` ב-GitHub. כדי לחזור אליה:
עבור לענף `v1-original` והעתק ממנו את התיקייה `src/`, את `index.html`
ואת `vite.config.js` בחזרה לענף הראשי. הנתונים שנרשמו לא ייפגעו —
סכמת הנתונים בגרסה 2 רק *הוסיפה* טבלאות ולא שינתה קיימות.
...
├── fonts.js          # גופנים מקומיים (Rubik / Assistant / JetBrains Mono)
├── components/       # Dashboard, Workouts, Nutrition, Shakes, Body,
│                     # History, Records, Settings, ui
